...
for (const s of seeds) {
  const a = A.get(s).champ, b = B.get(s).champ;
...
const erfc = (x) => { const t = 1 / (1 + 0.5 * Math.abs(x)); const y = t * Math.exp(-x * x - 1.26551223 + t * (1.00002368 + t * (0.37409196 + t * (0.09678418 + t * (-0.18628806 + t * (0.27886807 + t * (-1.13520398 + t * (1.48851587 + t * (-0.82215223 + t * 0.17087277)))))))));
  return x >= 0 ? y : 2 - y; };
const p = erfc(z / Math.SQRT2);
console.log(`McNemar (championship, trial-level pairs)`);
console.log(`  A wins & B wins   ${bothWin}`);
...
for (const s of seeds) {
  ...
  e.a += A.get(s).champ; e.b += B.get(s).champ; e.n++;
}
...
console.log(`  A better in ${wins}/${seasons.length} seasons (worse in ${losses}) -- sign test p ~ ${(2 * Math.pow(0.5, seasons.length) * (wins === seasons.length ? 1 : NaN)).toExponential ? (wins === seasons.length ? (2 * Math.pow(0.5, seasons.length)).toExponential(2) : "see binomial") : ""}`);
