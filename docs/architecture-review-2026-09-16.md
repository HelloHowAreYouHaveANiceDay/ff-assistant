...
- **Model improvement (after the architecture is at target):** re-screen the feature library under
  ...
  **M3 DONE 2026-09-17 -- the LINEUP model under STRESS: the optimizer is exactly optimal, the value claim holds against both cheap baselines, and THREE defects were fixed, all of them the same defect -- A PLAYER RESOLVED BY NAME (`docs/lineup-stress-2026-09-17.md`):** new `scripts/lineup-stress.mjs` fuzzes `optimalLineup` against an EXACT dynamic program over (slot index, used-player bitmask) whose eligibility table is hand-written and does NOT import `src/draft/slots.ts` -- **18,000 cases over nine templates (ESPN, Yahoo superflex with `W/R/T`/`Q/W/R/T`/`DEF`/`BN`/two `IR`, single- and two-flex, no-K/DST, slash forms, an edited `flex_ok`, a starved template), 0 failures**, with the CONTROL firing (the pre-matroid slot-order greedy fails **209 of 2,700**). **THE THREE DEFECTS, found on a temp `db.backup()` copy of the live store and each fixed with its fault injection (`test/lineup-stress-robustness.test.ts`, 17 tests; `test/lineup-stress-fuzz.test.ts`, 10):** (D1) `assertStartersAvailable` resolved the started man with `roster.find(x => x.name === ...)`, so with two men of one name it decided by ROSTER ORDER -- the unavailable copy first made it **THROW on a legal lineup** (`ff copilot lineup` and the MCP tool both fail outright), the available copy first let a genuinely-benched bye man **pass unseen**, which is the one failure it exists to catch; it now checks EVERY man of that name and complains only when all are out, byte-identical on unique names. (D2) `optimalLineup`'s own "started but not available" flag had the same `find` and fired falsely in exactly that case; it now reads the SEATED object. (D3) the season-line fallback `p.proj / perWeek` was unguarded, so a man with no usable season projection made **`totalProj` NaN** -- the verb's headline number, serialising to JSON `null` -- while `basisNote` claimed he "fell back to the season projection divided by 17"; `Number.isFinite` now guards it on **both** callers (the winprob `toWp` path too) and such men are NAMED as carried at zero. Plus the same name-keying in the bench `reason` map and the winprob bench filter (which dropped BOTH men of a duplicated name, losing a rostered man from the result), and a new flag when one name fills two slots. **VALUE (1,896 team-weeks, 462233, 2018-2025, cross-checked to the printed digit against the repo's own harness, and hindsight reproduced to -0.011 of `fact_lineup_week.optimal_pts`): served 89.55 vs managers 89.64 (-0.09 [-1.26, 0.75], 5/8 seasons), floor 85.83 (-3.81), trailing-4 form 86.01 (-3.63), hindsight 102.11. Served beats BOTH cheap baselines 8/8 seasons (+3.72 [3.17,4.36] over the floor, +3.54 [2.59,4.36] over trailing-4), and trailing-4 does NOT beat the season line (+0.18, CI straddles zero, 4/8).** Two thirds of the 12.6-point gap to hindsight sits at WR (-4.99) and RB (-3.78); K is not a decision at all (0.10 from the ceiling). **CALIBRATION on 23,657 rostered player-weeks: coverage 0.814 (nominal 0.80), flat across all eight seasons (0.798-0.826) -- but the band is SHORT ON THE UPSIDE everywhere (11.8% above p90 vs 6.8% below p10, worst at RB 14.2% and on the bench 13.9%), and the "injury-designated" cell's 1.000 is DEGENERATE, not calibrated: probed directly, 420 of 420 designated player-weeks have actual exactly 0 and p10 exactly 0, so the interval contains zero trivially.** **STABILITY: nothing flips on the live roster at a 1% perturbation and 0.3% of the starting SET flips at 5% -- but one draw inside each man's own p10-p90 flips it 95% of the time at 6.5 projected points, i.e. the ordering is confident and the outcome is not, and the summary prints neither the band nor the 0.70-point margin that decided the FLEX.** The `winprob` objective is CONNECTED but a NULL on this roster: over all 13 weeks `epCostPts` is 0 and `winPct == epWinPct` every time. **DETERMINISM byte-identical apart from `assumptions.asOf`; `lineupRecommend` 1ms cold / 0.1ms warm, the whole verb 155-172ms, 404ms with the app unreachable (`FF_LIVE_READ_TIMEOUT_MS=1`) and no hang.** **SEVEN FINDINGS, none fixed (each is outside this pass's file ownership or is a model change):** the week-1 "1 starters unmatched, scored 0: De'Zhaun Stribling" is **NOT a name-key gap** -- `feat_player_week` holds an identically-spelled row with `pts IS NULL`, which the seeding join filters out, so the caveat that exists to tell a sync gap from a join gap cannot; there is **NO lineup-lock concept anywhere**, so a Thursday player whose game has kicked off is still a candidate; **`ownership.slot = 'IR'` is never read** (Yahoo 129048 has four such men, all currently covered by the injury path by luck); the lineup's fallback is the PRESEASON line/17 and **never reads the D18 `rosPerGame` blend** that `season.ts:503` uses, so two surfaces hold two per-week strengths for one man; a **STALE** feed is invisible (only an ABSENT one is reported -- the absent case IS a working, fault-injected positive control); `lineupNameKey` does not apply the DST alias; and the "REAL schedule" caveat cannot distinguish a live read from the stored fallback. **ESPN's own weekly projection could not be a historical arm and that is a measurement, not an assumption: `raw_espn_projection` holds 585 rows, all season 2026 week 2.** Nothing shipped; no lever or model changed; all served artifacts byte-identical (`data/weekly-artifact.json` md5 `5aa938ecd0dc73ab68fe9a0137de3cfd`); typecheck clean; `npm test` **1,086 tests, 0 failing (2 skipped), +27**; lint **46 warnings, zero new**; backtest **39.5% / 96%** with the per-season line byte-for-byte. The live store was read-only throughout and the running app was neither launched, killed nor driven.
  **WP19 DONE 2026-09-17 -- M3's three open model findings TAKEN UP: the served weekly BAND is now conformally calibrated ON the artifact (D32), the lineup fallback is the D18 blend (D33), and the lineup serve states its MARGIN (D34):** **(D32)** new optional `bandCalibration` block on the weekly artifact -- per-position MULTIPLICATIVE conformal scales on the served p10/p90 RATIOS, fitted TRAIN-ONLY out of fold by a SECOND player-grouped k-fold pass in `tools/train_weekly.py` (`--band-conformal-k 5`; the existing `--conformal-k` calibrates the CONDITIONAL heads and its shift rides in the baselines, so it moves p50 and could not be reused). **It is a SCALE and not D16's additive shift because the two-part p10 sits on the ZERO ATOM wherever P(zero week) > 0.10** -- an additive offset lifts every one of those off the floor and turns each ruled-out man's realised 0 into a below-p10 miss, which would take M3's degenerate injury cell from 0.0% to ~100%. **And M3's framing of the LOWER tail was wrong, diagnosed before acting: only 64% of the 23,657 scored rows claim a p10 above zero at all, and on those rows the pre-D32 lower miss was already 0.090** -- pooled 10% below p10 is unattainable against an atom, so the target is `P(y > s90*p90) = 0.10` pooled and `P(y < s10*p10 | p10>0) = 0.10`, and the artifact says so. Out of fold the upper tail lands on **0.100-0.101 at every position** (QB 0.108->0.101, RB 0.109->0.101, WR 0.100->0.100, TE 0.098->0.100); on one league's ACTUAL rosters the same change is smaller and the gap is recorded as the population difference it is (pooled coverage **0.814 -> 0.802**, tail asymmetry **0.050 -> 0.032**, upside miss 0.118 -> 0.115). **GATE, both arms from ONE trained fold set** (14 folds trained once calibrated, re-scored with the field stripped via `--reuse-artifacts`; the strip step reports 14 of 14 carried one): ESPN clause (b) cov(pts>0) **0.839 -> 0.833**, Yahoo **0.843 -> 0.837**, all three clauses PASS both ways at every position, decision metric **85.7569 / 90.6828** and **142.2626 / 153.9269** UNCHANGED to four decimals; the cost is **+0.0026 / +0.0020 pooled CRPS, almost all RB**, which is what widening a loss-optimal ladder to make the tail honest costs. **CONTROLS: the refit control reproduces the served artifact BYTE-FOR-BYTE** (`populationHash 7ca2e2be49fc5aa7`, 84,582 rows) and the Yahoo one likewise with `--band-conformal-k 0`, proving the flag's off-state IS the pre-D32 trainer; **the identity control** shows the promoted file differs from the served one in `bandCalibration` and the golden block it implies AND NOTHING ELSE, with golden `mean`/`p50` identical to **0.00e+0**; the **manager backtest is identical line for line** (89.32, 48.0%, -0.324 [-1.518, 0.66], 1,896 team-weeks); the vectorised mixture is checked against the scalar one on every fit; the golden block is fault-injected to REFUSE a consumer that ignores the field; the zero atom is asserted to survive scales up to 100x; and the rollback was REHEARSED on scratch copies for all four files. **(D33)** `perGameStrength` (`src/draft/rosBlend.ts`) is now the ONE definition read by `src/draft/season.ts` and by BOTH fallback callers in `lineupRecommend` -- the lineup used to price a man the projector had no row for at the preseason line over 17 while the simulator priced him at the D18 blend on the same context. **Neither historical harness can measure it, and the POSITIVE CONTROL is what says so rather than a green null:** the two new `fbPre`/`fbBlend` arms in `scripts/lineup-stress.mjs` report ZERO men whose two fallback numbers differ, because the men with no weekly row are exactly the men with no `season_line_pg` in that table; `inseason-backtest-lineup.mjs` falls back to `td_ppg` and its own header says so, so its identical before/after is a degeneracy control. Proved by unit test on a context that carries `rosPerGame`, both callers, plus a control with none. **(D34)** `LineupResultJson.contested` -- per slot the seated man, the best legal SITTING alternative, the margin, both bands and the margin as a fraction of the band; the caveat states the tightest one and the shared dispatcher prints the three closest, so `ff copilot lineup` and the MCP tool say the same thing. **It corrects M3's own quoted number:** the "0.70 points decided the FLEX" compared two men who BOTH START, so it decided a LABEL and not a decision -- the real marginal call is Loveland over Godwin, 2.21 points, 9.6% of a 22.9-point band, and the tightest genuine call is QB at 1.47, 6.9% of a 21.2-point band. **PROMOTED** (rollback files `*.pre-d32-2026-09-17.json`, four of them, md5s in D32); `WEEKLY_SERVE_SWITCHED_ON` deliberately does NOT move -- no frozen row carries `2026-09-18` yet (verified on the store), so D30's design and this calibration first appear together in week 4 as ONE model under ONE date, and `src/weekly/scorecard.ts` now stamps `bandCal` so the two are distinguishable in the series anyway. Live serve re-checked after the swap: total **91.4**, same eight men, same margins, bands moved, Loveland's p10 still exactly 0. Gates: typecheck clean; `npm test` green; lint 46 warnings, zero new; backtest **39.5% / 96%** with the per-season line byte-for-byte. Not done and named: **DST's band is NOT calibrated** (`tools/train_dst_stream.py` is outside this pass's ownership, and its asymmetry runs the other way).
