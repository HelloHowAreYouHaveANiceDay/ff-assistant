...
> A.J. Brown). Tuning knobs (all in `V2Config`): `starterReserve` (balance vs concentration; shipped 4),
> `maxShare` (0.25 cap per player), `premium`, `aggr`, plus a pluggable value table (OUR values via
