...
async function cmdBacktest(rest: string[]) {
  ...
  // CONSENSUS BLEND. Re-rank the draft board's ORDERING toward the FFToday expert consensus, read from
  // the `consensusBlend` LEVER (shipped default 1; `--consensus-blend 0` turns it off). The transform
  // lives in src/draft/consensusBlend.ts and is SHARED with the live board (src/data/assemble.ts) -- one
  // copy, so the arbiter validates exactly what ships. No-lookahead only, like the age/opportunity
  ...
  for (const yr of seasons) {
    ...
    // CONSENSUS BLEND. Re-rank the board's ORDERING toward FFToday (src/draft/consensusBlend.ts, shared
    // with the live board). AFTER rookie injection so the consensus can re-rank rookies too.
    if (applyConsensus) proj = applyConsensus(proj, yr);
...
async function cmdInseasonBacktest(rest: string[]) {
  const SCRIPTS: Record<string, string> = {
    ...
    handcuff: "scripts/inseason-backtest-handcuff.mjs",
  ...
  if (!script) {
    ...
    console.log("  drop       value-min vs depth-aware DROP selection, by realized rest-of-season lineup value");
