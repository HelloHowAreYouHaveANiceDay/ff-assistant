 ...
 *
 * EVERY MAP IS BUILT FROM `stg_player`, AND NOT FROM `player_xref`. That is not a style choice, it
 * is a measured necessity: the two tables hold DIFFERENT SURROGATE KEY SPACES for the same men. Of
 * the 7,961 gsis ids present in both, **7,902 disagree** on `player_sk` -- only 59 agree. The cause
 * is visible in `stgPlayer.ts`, which calls `resolveOrMint(db, { ..., ids: {} })` with an EMPTY id
 * bag, so staging matches on (name_key, birthdate) and mints a fresh key whenever that misses, while
 * `player_xref` was minted separately by `playerIds.ts`; `player_identity` has 22,814 rows against
 * staging's 11,966 and the two key ranges barely overlap.
 *
 * The consequence is silent and total: a resolver that goes through `player_xref` returns keys that
 * join NOTHING -- not `stg_player`, not `feat_player_season`, not `feat_player_week`. The first cut
 * of this file did exactly that and produced 1 (one) overlapping key between 1,283 resolved
 * participation players and the 2023 feature rows, while reporting a 75% resolution rate. The rate
 * was true; the keys were from the wrong space. This is recorded as a merge note rather than fixed
 * here, because the fix belongs in `src/data/stgPlayer.ts` and `src/data/identity.ts`, which this
 ...
 *   3. pfr, through player_ids.pfr_id -> (name_key, position) -> stg_player. Neither stg_player nor
 *      player_xref carries a pfr id, so this is the only route the snap-count feed has.
...
export function buildSourceResolver(db: DB): SourceResolver {
  const staged = db.prepare(
    "SELECT player_sk, name_key, position, team, gsis_id, espn_id, sleeper_id, fantasypros_id FROM stg_player",
  ).all() as { player_sk: number; name_key: string; position: string; team: string | null; gsis_id: string | null; espn_id: string | null; sleeper_id: string | null; fantasypros_id: string | null }[];
  ...
  // PFR ids live only in the raw crosswalk. Mapping them through (name_key, position) rather than
  // through a name alone keeps the same rule as everything else: a pfr id whose (name_key, position)
  // pair is shared by two staged players resolves to nobody.
  const byPfr = new Map<string, number | null>();
  try {
    for (const r of db.prepare(
      "SELECT pfr_id, name_key, position FROM player_ids WHERE pfr_id IS NOT NULL AND pfr_id != ''",
    ).all() as { pfr_id: string; name_key: string; position: string }[]) {
      const sk = byNamePos.get(`${r.name_key}|${normPos(r.position ?? "")}`);
      if (sk != null) put(byPfr, r.pfr_id, sk);
    }
  } catch { /* a store without player_ids simply has no pfr route */ }
